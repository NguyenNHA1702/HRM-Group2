<%--
  Created by IntelliJ IDEA.
  User: brave
  Date: 5/17/2026
  Time: 8:03 PM
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/sidebar.css">

<aside class="sidebar">
    <!-- Brand -->
    <div class="sidebar-brand">
        <div class="brand-icon">
            <svg width="20" height="20" fill="none" stroke="white" stroke-width="2.2"
                 stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                <path d="M9 12h6M9 16h6M17 3H7a2 2 0 00-2 2v14a2 2 0 002 2h10a2 2 0 002-2V5a2 2 0 00-2-2z"/>
                <path d="M9 8h6"/>
            </svg>
        </div>
        <div class="brand-text">
            <span class="brand-name">HRMS</span>
            <span class="brand-sub">Quản lý Nhân sự</span>
        </div>
    </div>

    <!-- Navigation -->
    <nav class="sidebar-nav">
        <ul>
            <!-- Dashboard — all roles -->
            <li class="nav-item ${activeMenu == 'dashboard' ? 'active' : ''}">
                <a href="${pageContext.request.contextPath}/dashboard">
                    <svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1.5"/>
                        <rect x="14" y="3" width="7" height="7" rx="1.5"/>
                        <rect x="3" y="14" width="7" height="7" rx="1.5"/>
                        <rect x="14" y="14" width="7" height="7" rx="1.5"/>
                    </svg>
                    Dashboard
                </a>
            </li>

            <!-- Nhân viên: ADMIN, HR, MANAGER -->
            <c:if test="${sessionScope.roleGroup == 'ADMIN' || sessionScope.roleGroup == 'HR' || sessionScope.roleGroup == 'MANAGER'}">
                <li class="nav-item ${activeMenu == 'employees' ? 'active' : ''}">
                    <a href="${pageContext.request.contextPath}/employees">
                        <svg viewBox="0 0 24 24">
                            <path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/>
                            <circle cx="9" cy="7" r="4"/>
                            <path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/>
                        </svg>
                        Nhân viên
                    </a>
                </li>
            </c:if>

            <!-- Phòng ban: ADMIN, HR -->
            <c:if test="${sessionScope.roleGroup == 'ADMIN' || sessionScope.roleGroup == 'HR'}">
                <li class="nav-item ${activeMenu == 'departments' ? 'active' : ''}">
                    <a href="${pageContext.request.contextPath}/departments">
                        <svg viewBox="0 0 24 24">
                            <path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/>
                            <polyline points="9 22 9 12 15 12 15 22"/>
                        </svg>
                        Phòng ban
                    </a>
                </li>
            </c:if>

            <!-- Chấm công — all roles -->
            <li class="nav-item ${activeMenu == 'attendance' ? 'active' : ''}">
                <a href="${pageContext.request.contextPath}/attendance">
                    <svg viewBox="0 0 24 24">
                        <circle cx="12" cy="12" r="10"/>
                        <polyline points="12 6 12 12 16 14"/>
                    </svg>
                    Chấm công
                </a>
            </li>

            <!-- Nghỉ phép — all roles -->
            <li class="nav-item ${activeMenu == 'leave' ? 'active' : ''}">
                <a href="${pageContext.request.contextPath}/leave">
                    <svg viewBox="0 0 24 24">
                        <rect x="3" y="4" width="18" height="18" rx="2"/>
                        <line x1="16" y1="2" x2="16" y2="6"/>
                        <line x1="8" y1="2" x2="8" y2="6"/>
                        <line x1="3" y1="10" x2="21" y2="10"/>
                    </svg>
                    Nghỉ phép
                </a>
            </li>

            <!-- Lương — ADMIN, HR, EMPLOYEE (không hiện cho MANAGER) -->
            <c:if test="${sessionScope.roleGroup != 'MANAGER'}">
                <li class="nav-item ${activeMenu == 'payroll' ? 'active' : ''}">
                    <a href="${pageContext.request.contextPath}/payroll">
                        <svg viewBox="0 0 24 24">
                            <line x1="12" y1="1" x2="12" y2="23"/>
                            <path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/>
                        </svg>
                        Lương
                    </a>
                </li>
            </c:if>

            <!-- Admin only -->
            <c:if test="${sessionScope.roleGroup == 'ADMIN'}">
                <li class="nav-section">Quản trị</li>
                <li class="nav-item ${activeMenu == 'users' ? 'active' : ''}">
                    <a href="${pageContext.request.contextPath}/admin/users">
                        <svg viewBox="0 0 24 24">
                            <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/>
                            <circle cx="12" cy="7" r="4"/>
                        </svg>
                        Quản lý Users
                    </a>
                </li>
                <li class="nav-item ${activeMenu == 'roles' ? 'active' : ''}">
                    <a href="${pageContext.request.contextPath}/admin/roles">
                        <svg viewBox="0 0 24 24">
                            <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
                        </svg>
                        Phân quyền
                    </a>
                </li>
                <li class="nav-item ${activeMenu == 'settings' ? 'active' : ''}">
                    <a href="${pageContext.request.contextPath}/admin/settings">
                        <svg viewBox="0 0 24 24">
                            <circle cx="12" cy="12" r="3"/>
                            <path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83
                        0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09
                        A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06
                        A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09
                        A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06
                        A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09
                        a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06
                        A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/>
                        </svg>
                        Cấu hình
                    </a>
                </li>
            </c:if>
        </ul>
    </nav>

    <!-- User + Logout -->
    <div class="sidebar-footer">
        <div class="sidebar-user">
            <div class="user-avatar">
                <c:choose>
                    <c:when test="${not empty sessionScope.avatarUrl}">
                        <img src="${sessionScope.avatarUrl}" alt="avatar">
                    </c:when>
                    <c:otherwise>
                        ${sessionScope.fullName.substring(0,1)}
                    </c:otherwise>
                </c:choose>
            </div>
            <div class="user-info">
                <span class="user-name">${sessionScope.fullName}</span>
                <span class="user-role">${sessionScope.roleName}</span>
            </div>
            <a href="${pageContext.request.contextPath}/logout" class="logout-icon" title="Đăng xuất">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                     stroke-linecap="round" stroke-linejoin="round">
                    <path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/>
                    <polyline points="16 17 21 12 16 7"/>
                    <line x1="21" y1="12" x2="9" y2="12"/>
                </svg>
            </a>
        </div>
    </div>
</aside>

