...
public class UserDAO {
    ...
    // =========================================================
    // FIND employee by email (work hoac personal)
    // =========================================================
    ...
    // =========================================================
    // TAO employee toi thieu neu chua co (giu employee_id NOT NULL)
    // =========================================================

    private int createMinimalEmployee(String fullName, String email) throws SQLException {
        // Ma NV: "EMP" + 10 chu so epoch ms -> dam bao unique
    ...
    /**
     * Tao tai khoan moi voi bat ky email nao.
     * - Neu email da co trong employees -> lien ket luon.
     * - Neu chua co -> tu dong tao ho so nhan vien toi thieu, khong can chinh DB.
     */
    public void createUser(String fullName, String email,
                           int roleId, String rawPassword) throws SQLException {

        // Buoc 1: tim hoac tao employee
        int employeeId = findEmployeeIdByEmail(email);
        if (employeeId == -1) {
            employeeId = createMinimalEmployee(fullName, email);
        }

        // Buoc 2: kiem tra email chua duoc dung lam username
    ...
    public List<Object[]> getRoleGroups() throws SQLException {
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            while (rs.next()) {
                list.add(new Object[]{ rs.getInt("id"), rs.getString("display") });
            }
            return list;
        }
    }
    ...
    private UserAccountDTO mapRow(ResultSet rs) throws SQLException {
        ...
        if (ts != null) dto.setLastLoginAt(ts.toLocalDateTime());
        return dto;
    }
